...
//驗證帳號密碼 //認證
const isAuthenticated = ({ account, password }) => {
    return (
        //解析 
        //獲取資料
        //findIndex()方法返回在通過測試（作為一個功能提供）數組的第一個元素的索引。
        //findIndex()返回數值, true的話返回 > 0 or 1, 不存在返回-1
        getUsersDb().users.findIndex(user => user.account === account && user.password === password
        ) !== -1
    );
    // return account === 'admin@123.com' && password === 'admin'
};

const isExist = ( account ) => {
    return (
        //解析 
        //獲取資料
        //findIndex()方法返回在通過測試（作為一個功能提供）數組的第一個元素的索引。
        //findIndex()返回數值, true的話返回 > 0 or 1, 不存在返回-1
        getUsersDb().users.findIndex(user => user.account === account) !== -1
    );
    // return account === 'admin@123.com' && password === 'admin'
};
...
server.post('/auth/login', (request, response) => {
    ...
    if (isAuthenticated({ account, password })) {
        ...
        //401 沒有權限,帳號密碼不正確
        const status = 401;
        const message = "Incorrect account or password";
...
server.post('/auth/register', (req, res) => {
    ...
    fs.readFile(path.join(__dirname, 'users.json'), (err, _data) => {
        ...
        //Add new user
        data.users.push({ id: last_item_id + 1, account, password, name, type });//add some data
    ...
    const jwToken = createToken({ name, type, account });
